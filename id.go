...
func GenIdentity(dNum int, dName string, dManu string, creteAt int64) (string, error) {
	...
	//fmt.Println("1:", res)
	hash1 := Hash(res, sha256.New)
	//fmt.Println("2:", hash1)
	res = append(res, hash1...)
	hash2 := Hash(res, sha256.New)
	//fmt.Println("3:", hash2)
	hash3 := Hash(hash2, ripemd160.New)
	//fmt.Println("4:", hash3)
...
func GenBtcAddress(pk string) (string, error) {
	h1 := Hash([]byte(pk), sha256.New)
	pkHash := Hash(h1, ripemd160.New)
	var h2 []byte
	h2 = append(h2, []byte("0x00")...)
	h2 = append(h2, pkHash...)
	h3 := Hash(h2, sha256.New)
	h3 = Hash(h3, sha256.New)
	var h4 []byte
